...
resource "aws_instance" "web_app" {
  ...
  instance_type = "t3.micro"              # Staging tier instance (cheaper)
  subnet_id     = aws_subnet.public_1.id
  vpc_security_group_ids = [
    aws_security_group.web_sg.id
  ]
  associate_public_ip_address = true
  key_name                    = "hms-staging-key"
...
resource "aws_db_instance" "mysql" {
  ...
  engine                 = "mysql"
  engine_version         = "8.0"
  instance_class         = "db.t3.micro" # Staging tier db instance (cheaper)
  username               = "hms_admin"
  password               = "securestagingpassword123"
  db_subnet_group_name   = aws_db_subnet_group.db_subnets.name
  vpc_security_group_ids = [aws_security_group.db_sg.id]
  skip_final_snapshot    = true
  multi_az               = false # Single AZ for staging to save cost
